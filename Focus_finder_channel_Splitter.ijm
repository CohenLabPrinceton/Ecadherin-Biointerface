...
function processFolder(input) {
	...
	for (i = 0; i < list.length; i++) {
		...
		if(endsWith(list[i], suffix))
			processFile(input, ch1output, ch2output, list[i]);
	}
}

function processFile(input, ch1output, ch2output, file) {
	...
	run("Find focused slices", "select=100 variance=0.000 log");
	IJ.renameResults("Find_Focus","Results");
	slice = getResult('Slice',0);
	endslice=slice+6;
	selectWindow(Ch1name);
	run("Duplicate...", "duplicate range="+slice+"-"+endslice);
	C1Id=getTitle();
	stacknumber=nSlices;
	Stack.setDimensions(1, 1, stacknumber);
